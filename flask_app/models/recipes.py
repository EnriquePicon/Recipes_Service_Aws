...
class Recipe:
    ...
    def save(cls, formulario):
        ...
        nuevoId = connectToMySQL('recetas_recipes').query_db(query, formulario)
        return 

    @classmethod
    def get_all(cls):
        query = "SELECT recipes.*, first_name FROM recipes LEFT JOIN users ON users.id = recipes.user_id" #LEFT JOIN users
        results = connectToMySQL('recetas_recipes').query_db(query) #Lista de diccionarios
        recetas = []
    ...
    def get_by_id(cls, formulario): #recibir formulario_receta
        ...
        result = connectToMySQL('recetas_recipes').query_db(query, formulario) #recibimos una lista 
